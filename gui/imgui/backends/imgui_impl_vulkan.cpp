...
static void ImGui_ImplVulkan_LockQueue()
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (!bd)
        return;
...
/* [tetra]: Add queue locking */
static void ImGui_ImplVulkan_UnlockQueue()
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    if (!bd)
        return;
...
void ImGui_ImplVulkan_RemoveTexture(VkDescriptorSet descriptor_set)
{
    ...
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    VkDescriptorPool pool = bd->DescriptorPool ? bd->DescriptorPool : v->DescriptorPool;
    // [tetra]: HACK: Fix Validation Error: [ VUID-vkFreeDescriptorSets-pDescriptorSets-00309 ] | MessageID = 0xbfce1114
    {
        ImGui_ImplVulkan_LockQueue();
        VkResult err = vkQueueWaitIdle(v->Queue);
        ImGui_ImplVulkan_UnlockQueue();
